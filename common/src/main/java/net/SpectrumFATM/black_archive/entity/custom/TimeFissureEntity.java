...
public class TimeFissureEntity extends LivingEntity {
    ...
    public void aggrovate() {
        ...
        EntityType<?>[] possibleEntities = new EntityType<?>[]{
                ModEntityRegistry.DALEK.get(), ModEntityRegistry.CYBERMAT.get(), ModEntityRegistry.CYBERMAN.get()
        };
    ...
    private void spawnAdditionalTimeFissures(int count) {
        ...
        for (int i = 0; i < count; i++) {
            TimeFissureEntity fissureEntity = new TimeFissureEntity(ModEntityRegistry.TIME_FISSURE.get(), world);
